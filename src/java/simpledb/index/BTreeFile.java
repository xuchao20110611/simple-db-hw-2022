...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid,
        ...
        BTreeRootPtrPage root_ptr_page = null;
        try {
            root_ptr_page = getRootPtrPage(tid, dirtypages);
        } catch (IOException e) {
            e.printStackTrace();
        }
        BTreePageId page_pid = root_ptr_page.getRootId();
        while (page_pid.pgcateg() != BTreePageId.LEAF) {
            BTreeInternalPage page = (BTreeInternalPage) getPage(tid, dirtypages, page_pid, Permissions.READ_ONLY);
            if (f == null) {
                page_pid = page.iterator().next().getLeftChild();
                dirtypages.put(page_pid, getPage(tid, dirtypages, page_pid, Permissions.READ_ONLY));
            } else {
                ...
                while (it.hasNext()) {
                    ...
                    if (f.compare(Op.LESS_THAN_OR_EQ, entry.getKey())) {
                        page_pid = entry.getLeftChild();
                        dirtypages.put(page_pid, getPage(tid, dirtypages, page_pid, Permissions.READ_ONLY));
                        break;
                    }
                }
                if (entry != null && f.compare(Op.GREATER_THAN, entry.getKey())) {
                    page_pid = entry.getRightChild();
                }
            }
        }
        dirtypages.put(page_pid, getPage(tid, dirtypages, page_pid, perm));
        return (BTreeLeafPage) getPage(tid, dirtypages, page_pid, perm);
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        if (page.getLeftSiblingId() != null) {
            left = (BTreeLeafPage) getPage(tid, dirtypages, page.getLeftSiblingId(),
        ...
        if (left != null) {
            left.setRightSiblingId(newPage.getId());
        }
        // ite must have one element, as long as newpage has no greater element than
        // page
        Field f = tuple.getField(keyField);
        BTreeEntry entry = new BTreeEntry(f, newPage.getId(),
                page.getId());
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        entry = ite.next();
        BTreePageId parent_id = page.getParentId();
        // BTreeEntry entry = ite.next();
        page.deleteKeyAndRightChild(entry);
        entry.setLeftChild(newPage.getId());
        entry.setRightChild(page.getId());
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, parent_id, entry.getKey());

        parent.insertEntry(entry);
        updateParentPointers(tid, dirtypages, newPage);
        updateParentPointer(tid, dirtypages, parent_id, newPage.getId());
        updateParentPointer(tid, dirtypages, parent_id, page.getId());
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        if (num < 0) {
            throw new DbException("BTreeFile::stealFromLeafPage: num<0");
        }
        num = num / 2;
        // parent.deleteKeyAndRightChild(entry);
        if (isRightSibling) {
            Iterator<Tuple> ite = sibling.iterator();
            Tuple tuple = null;
            while (num > 0) {
                tuple = ite.next();
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        num = num / 2;
        if (num < 0) {
            throw new DbException("BTreeFile::stealFromLeftInternalPage: num<0");
        }
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        if (num < 0) {
            throw new DbException("BTreeFile::stealFromRightInternalPage: num<0");
        }
        num = num / 2;
    ...
    public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        parentEntry.setRightChild(rightPage.iterator().next().getRightChild());
